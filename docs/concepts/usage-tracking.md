...
- Control UI: the chat composer's context ring popover shows **plan usage** for subscription providers — per-window bars (5-hour, weekly, model-scoped) with reset times, the provider plan when known (for example `Max (20x)`), and extra-usage credits. Sessions billed through a plan hide per-token dollar estimates; API-billed sessions keep `Est. cost` and the cost-by-type breakdown. Claude Code CLI (`claude-cli`) subscription windows are reported as a separate **Claude Code** usage row (see below).
...
- **Claude Code** (`claude-cli`): usage follows the route an agent runs on.
  ...
  the `rate_limit_event` Claude Code streams on each local turn under the
  Gateway host's own login. It is reported beside the Anthropic row, never
  merged into it, because an Anthropic credential can be a pay-per-use key or
  a different account. The windows are held in Gateway memory until their
  reset time, so the row appears after the first such turn following a Gateway
  start and reaches Gateway clients (`usage.status`, `/status`), not a separate
  ...
  (`observedAt` in `usage.status`, shown as `as of ...` in `/status` and as
  `Observed ...` on the Control UI usage cards), and
